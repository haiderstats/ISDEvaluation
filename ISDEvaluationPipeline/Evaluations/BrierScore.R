...
BrierScore = function(survMod, type = "Integrated", singleTime = NULL, integratedTimes = NULL, numPoints=NULL){
  ...
  score = ifelse(type =="Single",
                 singleBrier(survMod, singleTime),
                 integratedBrier(survMod, integratedTimes, numPoints))
  return(score)
}

singleBrier = function(survMod, BrierTime = NULL){
  ...
  #Default brier time will be the median survival time generated by the Kaplan Meier curve.
  if(is.null(BrierTime)){
    KMCurve = prodlim(Surv(trainingEventTimes, trainingCensorStatus)~1)
    BrierTime = tryCatch({
